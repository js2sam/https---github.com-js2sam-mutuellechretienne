...
function checkCookie() {
  ...
  if (RecuperationCookies !== null && RecuperationCookies.status !== "") {
    //on zappe le tableau du bareme
    nextPrev(1);
    //$('#scrpt_amgs_' + RecuperationCookies.status).prop("checked", true);
...
function gestionCookies(etat) {
  ...
  if (etat == 'save') {
    //assigation au cookies generaux
...
window.onload = () => { 
  var jsonConnect = JSON.parse(getCookie('userinfo'))
